...
class ExperimentParams:
    ...
    fc_hz: float = 1100.0
    ft_hz: float = 1200.0
    noise_level_dbfs: float = -20.0


    Lt_start_dbfs: float = np.ceil(noise_level_dbfs - 10*np.log10(fc_hz)) + 25
    Lt_min_dbfs: float = -100.0
    Lt_max_dbfs: float = -3.0

    # --- Staircase steps ---
    phase1_step_db: float = 2.0   # 初期フェーズ(1up-1down)の上下ステップ
    phase2_step_db: float = 1.0   # 反転後フェーズ(1up-2down)の上下ステップ
    max_trials: int = 60
    max_reversals: int = 6
    ...
    # ExperimentParams に追加
    target_label: str = "signal2"  # "signal1" にすれば元の振る舞いに戻せます
...
def cosine_ramp(buf: "np.ndarray", ramp_samp: int) -> "np.ndarray":
    ...
    # 立ち上がり: cos^2(θ) を 90°→0°に走査 → 0→1 へ
    theta = np.linspace(math.pi/2, 0, ramp_samp)
    rise = (np.cos(theta))**2

    # 立ち下がり: cos^2(θ) を 0°→90°に走査 → 1→0 へ
    theta = np.linspace(0, math.pi/2, ramp_samp)
    fall = (np.cos(theta))**2
...
class StairState:
    ...
    trial_index: int = 0
    reversals: int = 0
    last_direction: Optional[int] = None  # +1 up, -1 down
    reversal_levels: List[float] = field(default_factory=list)
    consecutive_correct: int = 0
    phase: int = 1                        # 1: 1up-1down, 2: 1up-2down
    current_step_db: float = 2.0          # 初期は phase1_step_db
    
class OneUpTwoDownLt:
    """
    Phase 1: 1up-1down, step = phase1_step_db
    Phase 2 (after first reversal): 1up-2down, step = phase2_step_db
    """
    def __init__(self, p: ExperimentParams):
        self.p = p
        start = max(min(p.Lt_start_dbfs, p.Lt_max_dbfs), p.Lt_min_dbfs)
        self.state = StairState(Lt_db=start, current_step_db=p.phase1_step_db)

    def _apply_step(self, go_up: bool):
        s, p = self.state, self.p
        step = s.current_step_db
        if go_up:
            old = s.Lt_db
            s.Lt_db = min(s.Lt_db + step, p.Lt_max_dbfs)
            direction = +1
        else:
            old = s.Lt_db
            s.Lt_db = max(s.Lt_db - step, p.Lt_min_dbfs)
            direction = -1
    ...
    def update(self, correct: bool):
        ...
        if s.phase == 1:
            # --- Phase 1: 1up-1down ---
            if correct:
                old, direction = self._apply_step(go_up=False)   # down
            else:
                old, direction = self._apply_step(go_up=True)    # up
        ...
        if direction is not None and s.last_direction is not None and direction != s.last_direction:
            ...
            if s.phase == 1:
                s.phase = 2
                s.current_step_db = p.phase2_step_db
...
def fade_out(buf: np.ndarray, sr: int, ramp_ms: float = 20.0) -> np.ndarray:
    ramp_samp = int(ramp_ms * sr / 1000)
    if ramp_samp > len(buf):
        ramp_samp = len(buf)
    theta = np.linspace(0, np.pi/2, ramp_samp)
    fall = (np.cos(theta))**2
    buf[-ramp_samp:] *= fall
    return buf

def assemble_two_interval_buffer(sig_first: np.ndarray,
                                 sig_second: np.ndarray,
                                 sr: int,
                                 pre_ms: float = 500.0,
                                 isi_ms: float = 250.0,
                                 tail_ms: float = 50.0) -> np.ndarray:
    pre  = np.zeros(int(pre_ms * sr / 1000), dtype=np.float32)
    isi  = np.zeros(int(isi_ms * sr / 1000), dtype=np.float32)
    tail = np.zeros(int(tail_ms * sr / 1000), dtype=np.float32)

    # 型合わせ
    sig_first  = sig_first.astype(np.float32, copy=False)
    sig_second = sig_second.astype(np.float32, copy=False)

    return np.concatenate([pre, sig_first, isi, sig_second, tail], axis=0)
...
def run_trial(p,stair,writer):
    ...
    if not AUDIO_OK or p.FORCE_SIMULATION:
        ...
        # ターゲット（正解）は Signal2 がある区間
        target_interval = 3 - signal_interval  # CHANGED

        buf = assemble_two_interval_buffer(first, second, p.sample_rate,
                                   pre_ms=p.pre_silence_ms, isi_ms=500.0, tail_ms=250.0)
        print(f"\nTrial {s.trial_index} (Lt={Lt:.2f} dBFS)")
        play(buf,p.sample_rate)
        ans=prompt_user(f"Which interval had the TARGET? {p.response_keys[0]} or {p.response_keys[1]}: ")
        
        if ans not in p.response_keys:
            resp_interval = 0
            correct = False
        else:
            resp_interval = 1 if ans == p.response_keys[0] else 2
            correct = (resp_interval == target_interval)  # CHANGED
        
    if p.feedback: print("✓ Correct!" if correct else "✗ Incorrect.")

    reversal_flag = stair.update(correct)
    ts=time.strftime("%Y-%m-%d %H:%M:%S")
    # ログにも target_interval を入れると後で検証しやすい（任意）
    writer.writerow([
        s.trial_index,
        f"signal1_interval={signal_interval}, target=signal2, target_interval={target_interval}",  # CHANGED (condition)
        resp_interval, int(correct), p.fc_hz, p.ft_hz, f"{Lt:.2f}", int(reversal_flag), ts
    ])
...
def main():
    ...
    writer.writerow(["trial", "condition", "response", "correct", "fc", "ft", "Lt", "reversal", "timestamp"])
